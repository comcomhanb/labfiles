          ...
          if (accounts.length > 0) {
             ...
             mobileSdk.custom.BankingApi.post('accounts/' + account.id + '/transactions', paymentBody, {inType: 'json'}, null).then(
                function (result) {
                    ...
                    conversation.reply({ text: '요청 하신 금액 인, ' + amount + '을 ' + fromAccountType + '로 부터' + toAccount + '으로 송금 했습니다.' });
                    conversation.transition();
                    done();
                },
                function (error) {
                  conversation.reply({ text: '요청 하신 금액 인, ' + amount + '을 ' + fromAccountType + '로 부터' + toAccount + '으로 보낼 수 없습니다.'});
                  conversation.transition();
                  done();
                }
             )
