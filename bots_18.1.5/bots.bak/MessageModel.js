...
const message_buttonSchema = Joi.object().keys({
  label: Joi.string().required().trim(),
  url: Joi.string().uri().required(),
});

const buttonsSchema = Joi.array().items(Joi.string());

const keyboardSchema = Joi.object().keys({
  type: Joi.string().required().valid('buttons'),
  buttons: buttonsSchema.required(),
});
...
const messageSchema = Joi.object().keys({
  ...
  message_button: message_buttonSchema.optional(),
});

const kakaoConversationMessageSchema = Joi.object().keys({
...
const MessageModel = class {
  ...
  //Kakao Object Start 
  static photoObject(url, width, height){
    var instance = {
      url: url    
    };
  ...
  static message_buttonObject(label, url){
    var instance = {
      label: label    
    };
  ...
  // Kakao Object End
    ...
    /** kakaoConversationMessage */
  static kakaoConversationMessage(text, photo, button, keyboard) {
      var response = {
          message : {
          }
      };
      ...
      if (button) {
        response.message.message_button = button;
      }
      if (keyboard) {
